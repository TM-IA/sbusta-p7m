...
/**
 * Entry point registered for ACTION_VIEW on application/pkcs7-mime and
 * application/x-pkcs7-mime (see AndroidManifest.xml). Mirrors the
 * desktop wrappers' summary step: shows signer metadata, then lets the
 * user open the extracted PDF or export the metadata as JSON — no
 * automatic file-next-to-source save, Android's sandboxed storage
 * model doesn't support that the way a desktop filesystem does (see
 * plan decisions).
 */
class SbustaP7mActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val testoStato = findViewById<TextView>(R.id.testo_stato)
        val testoFirmatari = findViewById<TextView>(R.id.testo_firmatari)
        val pulsanteApriPdf = findViewById<Button>(R.id.pulsante_apri_pdf)
        val pulsanteEsportaJson = findViewById<Button>(R.id.pulsante_esporta_json)
